...
def test_builds_correct_url_and_params():
    ...
    with patch("check_permits.requests.get", return_value=resp) as mock_get:
        check_availability("300009", "2026", "07")

    args, kwargs = mock_get.call_args
    assert args[0] == check_permits.availability_url("300009")
    assert "300009" in args[0]
    assert kwargs["params"] == {"year": "2026", "month": "07", "inventoryBucket": "FIT"}
    assert kwargs["timeout"] == 15
    assert "User-Agent" in kwargs["headers"]


def test_url_includes_facility_id():
    resp = _ok_response()
    with patch("check_permits.requests.get", return_value=resp) as mock_get:
        check_availability("233273", "2026", "08")
    assert "233273" in mock_get.call_args[0][0]
...
def test_retries_then_succeeds():
    ...
    with (
        ...
        result = check_availability("300009", "2026", "07")
